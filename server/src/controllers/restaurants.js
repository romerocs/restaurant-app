...
module.exports = {
  ...
  list(req, res) {
    return Restaurants.findAll({
      ...
      .then(restaurants =>
        res.status(200).send({ content: restaurants, type: "directoryList" })
      )
      .catch(error => res.status(400).send(error));
  },
  category(req, res) {
  ...
  random(req, res) {
    return Restaurants.findOne({
      where: {
        category: req.params.category,
        meal: { [Symbol.for("sequelize.operator.contains")]: [req.params.meal] }
      },
    ...
    return Restaurants.findOne({
      ...
      .then(restaurants => {
        ...
        return restaurants
          ...
          .then(updatedRest => res.status(200).send(updatedRest))
          .catch(error => res.status(400).send(error));
      })
      .catch(error => res.status(400).send(error));
  },
